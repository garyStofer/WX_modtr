...
WORD execNameValueCmd(BYTE * name, BYTE * value, BYTE user) {
    ...
    /////////////////////////////////////////////////
    // We received a Weather station related command 
    else if (name[0] == CMDGROUP_WX)
    {
	    WORD tmp2;
	    ...
	    switch(name[1]) 
	    {
	    	...
	    	case CMDCODE_WX_Elev:
	    		tmp2= atoi(value); 
	    		tmp = tmp2 &0xff;
	    		appcfgPutc( APPCFG_WX_STATION_ELEVL, tmp);
	    		tmp2 = tmp2 >>8;
	    		tmp= tmp2&0xff;
    ...
    else if (name[0] == CMDGROUP_XBRD)
    {
        ...
        if (name[1] == 'r') {
            ...
            //IOR5E has relays 1 to 5
             if ((tmp==XBRD_TYPE_IOR5E) && (ch>=1) && (ch<=5) ) {
        ...
        //Output command, like "xo5=0" to clear digital output 5 for example
            ...
            if ((tmp==XBRD_TYPE_IOR5E) && (ch>=1) && (ch<=6) ) {
        ...
        {
	        if (name[2] == 'Y' )	// set the year
        		RTC.Year = atoi((char *)value);
	        else if(name[2] == 'M' ) 
	        	RTC.Month = atoi((char *)value);		
           	else if(name[2] == 'D' ) 
        		RTC.Date = atoi((char *)value);
           	else if(name[2] == 'H' ) 
	        	RTC.Hour = atoi((char *)value);
	        else if(name[2] == 'm' ) 
	        	RTC.Min = atoi((char *)value);
	         else if(name[2] == 's' )
	         { 
	        	RTC.Sec = atoi((char *)value);
	        	SetRTC();
	         }
	    } 		
    }
    ...
    *pGetTagInfo->val = '\0';
    ...
    else if (tagGroup == VARGROUP_GENERAL)
    {
        ...
        //Returns NetBIOS name
            ...
            pGetTagInfo->ref = cmdGetEepromStringVar(ref, pGetTagInfo->val, APPCFG_WX_PASSWD);
            return 1;   //One byte was written
        }   
        else if ( (tagVal >= VARVAL_GEN_WX_IP0) && (tagVal <= VARVAL_GEN_WX_IP3) ) {
            //Return decimal value of requested WX IP address  byte
            pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val, appcfgGetc( APPCFG_WX_IP_ADDR0 + (tagVal-VARVAL_GEN_WX_IP0) ));
            return 1;   //One byte was written
        }    
        else if ( tagVal == VARVAL_GEN_WX_ALT )
        {	if (pGetTagInfo->ref == HTTP_START_OF_VAR )
	        {
		         alt = appcfgGetc( APPCFG_WX_STATION_ELEVH);
		         alt<<=8;
		         alt +=  appcfgGetc( APPCFG_WX_STATION_ELEVL);
	           
		     }
	         pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val,alt) ;
	         return 1;
        }
        else if (tagVal == VARVAL_GEN_WX_WNDDIR )
        {
           	 pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val,Wind_dir) ;
           	 return 1;
        }
        ...
        {
           	 pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val,Solar_rad) ;
           	 return 1;
        }
...
// GARY's RTC tag %x40- %x44 time components. year,month,day,hour,minute        
        else if (varVal4to7==0x40)
        {
	        if ( varVal0to3 == 0 )	// the year
	        { 	if (pGetTagInfo->ref == HTTP_START_OF_VAR )
		        {
		        		GetRTC();
	       	 	}    
	       	    pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val, RTC.Year);
	       	}
	       	else if (varVal0to3 == 1)// the month
	       			pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val, RTC.Month);
	       	else if (varVal0to3 == 2)// the day
	       			pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val, RTC.Date);
	       	else if (varVal0to3 == 3)// the Hour
	       			pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val, RTC.Hour);
	       	else if (varVal0to3 == 4)// the Minute
	       			pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val, RTC.Min);
	       	else if (varVal0to3 == 5)// the Second
	       			pGetTagInfo->ref = cmdGetWordVar(ref, pGetTagInfo->val, RTC.Sec);					
        	return 1;
