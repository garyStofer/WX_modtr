...
#endif
{
    ...
    if (INTCON_TMR0IF)
    {
        ...
        if (--tick1sCount == 0) 
	    {
            tick1sCount = TICKS_PER_SECOND; 
            tickSec++;
           
           	//  capture the timer3 counter every 1 second, input from the anemometer, 
           	//  1 revolution == 6 counts  
           	if (wnd_cnt_ndx >= WX_UPLINK_INTERVAL)  
        		wnd_cnt_ndx = 0;
        		                   
            Wind_counts[wnd_cnt_ndx++] = TMR3L;				// reading the "L" byte latches the "H"
            TMR3L = 0;										// Writing the "L" byte latches the "H"
...
//    WDTCON_SWDTEN = 1;
	...
	if (appcfgGetc(APPCFG_USART2_CFG & APPCFG_USART_ENABLE)) {
	        appcfgUSART2();              //Configure the USART1
	    }
#endif
