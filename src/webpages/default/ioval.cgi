...
<li>Inputs can only be monitored, and are represented by a 1 or 0 character.</li>
<li>Unavailable ports are hidden when in use by expansion modules</li></ul>
...
<tr>
	...
	lnb();
	lnb();
	if(xbd==2) lnb(); else ln(%c55,%c05,"cx5");
	lnb();
	lnb();
	if(xbd==2)lnb(); else ln(%c52,%c02,"cx2"); 
	if(xbd==2)lnb(); else ln(%c51,%c01,"cx1");
	if(xbd==2 || xbd==3 )lnb(); else ln(%c50,%c00,"cx0"); 
	</script>
</tr>
...
<tr>
	...
	ln(%g52,%g02,"gx2");
	ln(%g51,%g01,"gx1");
	if(xbd==2 || xbd==3)lnb(); 	else ln(%g50,%g00,"gx0");	/* hide the G0 button when Ior5e/WX uses it for RTC disable*/
	</script>
</tr>
