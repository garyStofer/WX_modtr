...
void appcfgCpuIO(void)
{
	...
	//If IOR5E is used, ensure B0, B1, B4 and B5 are inputs  Optocoplers 
    ...
    //G4 = Is connected to IOCHRDY
     val = appcfgGetc(APPCFG_TRISG) | 0x10;
...
void appcfgCpuIOValues(void)
{
    ...
    val = appcfgGetc(APPCFG_PORTG);
    // Make sure G0 is always high -- RTC clock disable 
    if (appcfgGetc(APPCFG_XBRD_TYPE) == XBRD_TYPE_IOR5E || appcfgGetc(APPCFG_XBRD_TYPE) == XBRD_TYPE_WX) 
	{
	    val |= 0x1;
	} 
...
ROM BYTE eeConfigArray[] = {
    ...
    0,                                  // Offset = 143
    0,                                  // Offset = 144
    38,				// Offset 145, Wunderground IP address byte 0
